...
import { ImageBackground, Platform, Text, View, useWindowDimensions } from 'react-native';
...
export default function WelcomeRoute() {
  ...
  const { authStatus, userId } = useVaanika();
  const { width } = useWindowDimensions();
  const isCompactWeb = Platform.OS === 'web' && width < 920;
    ...
    return (
      <WebShell pageBackgroundUri={WEB_IMAGES.pageBackground}>
        <View style={webStyles.pageGrid}>
          <View style={[webStyles.twoColumn, isCompactWeb && { flexDirection: 'column' }]}>
              ...
              >
                ...
                <View style={webStyles.heroContent}>
                  <Text
                    style={[
                      webStyles.heroTitle,
                      isCompactWeb && { fontSize: 34, lineHeight: 40, maxWidth: '100%' },
                    ]}
                  >
                    Speak naturally. Learn faster. Stay consistent.
                  </Text>
          ...
          <View style={webStyles.previewRow}>
            <View style={[webStyles.previewButtonWrap, isCompactWeb && { width: '100%' }]}>
              <SecondaryButton
